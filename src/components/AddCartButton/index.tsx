...
export default function AddCartButton({cart, handleAddItemToCart, decreaseQuantity, handleRemoveFromCart, calculateTotal}:any){
    
    let [widthCart, setWidth] = useState(Number);
    const [open, cycleOpen] = useCycle(true, false);

    let handleClick = () => {
        cycleOpen()
        setWidth(window.innerWidth > 768 ? 468 : 328)
    };
